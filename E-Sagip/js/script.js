const API_BASE_URL = 'https://e-sagip-production.up.railway.app';
...
async function loadVolunteers() {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/auth/volunteers`);
...
async function approveVolunteer(id) {
  try {
    await fetch(`${API_BASE_URL}/api/auth/volunteers/${id}/approve`, { method: 'PUT' });
    loadVolunteers();
  } catch (err) {
...
async function removeVolunteer(id) {
  ...
  try {
    await fetch(`${API_BASE_URL}/api/auth/volunteers/${id}`, { method: 'DELETE' });
    loadVolunteers();
  } catch (err) {
...
function handleVolunteerLogout() {
  if (confirm('Sign out of the volunteer portal?')) {
...
async function loadDashboardSummaryMetrics() {
  try {
    const response = await fetch(`${API_BASE_URL}/api/operations/dashboard-stats`);
    if (!response.ok) throw new Error('Failed to fetch dashboard metrics.');

    const stats = await response.json();

    const totalVolEl    = document.querySelector('.stat-value-v');
    const activeVolSub  = document.querySelector('.stat-card:nth-child(1) .stat-sub');
    const activeOpsEl   = document.querySelector('.stat-value-op');
    const enrolledSub   = document.querySelector('.stat-card:nth-child(2) .stat-sub');
    const footerVolEl   = document.getElementById('vol-num');

    if (totalVolEl)   totalVolEl.textContent  = stats.totalVolunteers;
    if (activeVolSub) activeVolSub.textContent = `${stats.activeVolunteers} active`;
    if (activeOpsEl)  activeOpsEl.textContent  = stats.activeOperations;
    if (enrolledSub)  enrolledSub.textContent  = `${stats.enrolledVolunteers} enrolled`;
    if (footerVolEl)  footerVolEl.textContent  = stats.totalVolunteers;
  } catch (err) {
...
async function loadLiveSkillsDistributionGraph() {
  try {
    ...
    document.querySelectorAll('.skills-card .skill-row').forEach(row => {
      ...
      const count = dataMap[labelEl.textContent.trim()] || 0;
      countEl.textContent = count;
      barEl.style.width   = `${(count / highestCount) * 100}%`;
    });
  } catch (err) {
    console.error('Skills distribution failed:', err);
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Schedule: no past dates ──────────────────────────────────
  const schedInput = document.getElementById('sched');
  if (schedInput) {
    const now       = new Date();
    const offset    = now.getTimezoneOffset() * 60000;
    const local     = new Date(now - offset);
    schedInput.min  = local.toISOString().slice(0, 16);
  }
  ...
  // ── None checkbox: disable all other skills ──────────────────
