...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (otherSkill) {
      await db.query(
        'INSERT INTO volunteer_other_skills (volunteer_id, description) VALUES (?, ?)',
        [volunteerId, otherSkill]
      );
    }
...
// 3. FETCH ALL VOLUNTEERS (Admin Dashboard)
  ...
  try {
    await db.query(
      `UPDATE volunteers
       ...
       WHERE id = ?`,
      [firstName, lastName, contactNumber, address, status, req.params.id]
    );
    res.json({ success: true });
  } catch (err) {
